...
use Etn_Pro\Utils\Helper as Helper_Pro;

defined( 'ABSPATH' ) || exit;

/**
 * eventin notice class.
 * Handles dynamically notices for lazy developers.
 *
 * @author themewinter team: alpha omega sigma
 * @since 1.0.0
 */
...
class Notice {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_eventin-notices', [ $this, 'dismiss' ] );
	}
	...
	public function enqueue_scripts() {
		?>
			...
			jQuery(document).ready(function ($) {
				$( '.eventin-notice.is-dismissible' ).on( 'click', '.notice-dismiss', function() {
					_this 		= $( this ).parents( '.eventin-active-notice' );
	...
	public static function push($notice) {
		...
		// Notice ID.
		$notice_id    = 'eventin-sites-notice-id-' . $notice['id'];
		$notice['id'] = $notice_id;
		if ( ! isset( $notice['id'] ) ) {
			$notice_id    = 'eventin-sites-notice-id-' . $notice['id'];
			$notice['id'] = $notice_id;
	...
	public static function markup( $notice = [] ) {
		?>
		<div id="<?php echo esc_attr( $notice['id'] ); ?>" class="<?php echo esc_attr( $notice['classes'] ); ?>" <?php echo Helper_Pro::render($notice['data']); ?>>
			<p>
				<?php 
				echo Helper_Pro::render( $notice['message'] ); ?>
			</p>
