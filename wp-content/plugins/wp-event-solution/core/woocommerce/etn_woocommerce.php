...
class Etn_Product_Data_Store_CPT extends WC_Product_Data_Store_CPT implements WC_Object_Data_Store_Interface, WC_Product_Data_Store_Interface {
    ...
    function mail_attende_report( $pdf_data ){
        ...
        if( is_array( $pdf_data ) &&  !empty( $pdf_data['update_key'] ) ){
            ...
            $attendee_post = $wpdb->get_col( $prepare_guery );
            if( is_array($attendee_post) && !empty( $attendee_post ) ){
                foreach ($attendee_post as $key => $value) {
                    ...
                    $edit_token     = get_post_meta( $value , 'etn_info_edit_token' , true );
                    ...
                    $attend_pdf_data  = array( 
                        'event_name'        => $pdf_data['event_name'] , 
                        'event_location'    => $pdf_data['event_location'] , 
                        'etn_ticket_price'  => $pdf_data['etn_ticket_price'] ,
                        'etn_start_date'    => $pdf_data['etn_start_date'] , 
                        'etn_end_date'      => $pdf_data['etn_end_date'] , 
                        'etn_start_time'    => $pdf_data['etn_start_time'] , 
                        'etn_end_time'      => $pdf_data['etn_end_time'] , 
                    ...
                    <div class="etn-attendee-details-button-parent">
                        ...
                        <div class="etn-attendee-details-button-download">
                            <form class="form-horizontal" id="" role="form" action="<?php echo esc_url( $action_url );?>" method="post">
                                <?php foreach ($attend_pdf_data as $key => $value) : ?>
                                    <input type="hidden" name="<?php echo esc_html( $key) ;?>" value="<?php  echo esc_html( $value); ?>" />
                                <?php endforeach; ?>
                                <input type="submit" class="etn-btn etn-success download-details" name="<?php echo esc_attr("etn_download_attendee_action");?>" value="<?php  echo esc_html__('Download Ticket','eventin');?>"/>
                                <?php  wp_nonce_field('attendee_ticket_nonce','attendee_ticket_data'); ?>
                            </form>
                        </div>
                        <div class="etn-attendee-details-button-edit">
                            <form action="<?php echo esc_url( $action_url );?>" method="post">
                                <input type="hidden" name="etn_info_edit_token" value="<?php  echo esc_html( $edit_token );?>"/>
                                <input type="hidden" name="attendee_id" value="<?php  echo esc_html( $attendee_id );?>"/>
                                <input type="submit" class="etn-btn etn-success edit-details" name="<?php echo esc_attr("etn_update_attendee_action");?>" value="<?php  echo esc_html__('Update Info','eventin');?>" />
                                <?php  wp_nonce_field('attendee_update_nonce','attendee_update_nonce'); ?>
                            </form>
                        </div>
                    </div>
                    <?php
